...
export default function EmailModal({ isOpen, onClose, advisory }: EmailModalProps) {
  ...
  const addIndividualEmail = () => {
    setIndividualEmails(prev => [...prev, '']);
  };

  const removeIndividualEmail = (index: number) => {
    setIndividualEmails(prev => prev.filter((_, i) => i !== index));
  };

  const updateIndividualEmail = (index: number, email: string) => {
    setIndividualEmails(prev => prev.map((e, i) => i === index ? email : e));
  };
  ...
  const getSelectedRecipientsCount = () => {
    if (sendMethod === 'clients') {
      ...
      return selectedClientObjects.reduce((total, client) => total + client.emails.length, 0);
    } else {
  ...
  const handleSendEmail = async () => {
    if (sendMethod === 'clients' && selectedClients.length === 0) {
      alert('Please select at least one client');
      return;
    }

    if (sendMethod === 'individual' && individualEmails.filter(e => e.trim()).length === 0) {
    ...
    try {
      const payload = {
        ...
        ...(sendMethod === 'clients' 
          ? { selectedClients }
          : { individualEmails: individualEmails.filter(e => e.trim()) }
        ),
        ...(isScheduled && {
          scheduledDate,
          scheduledTime
