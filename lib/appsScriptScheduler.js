...
// Conditionally create an HTTPS agent based on environment
// Only bypass SSL validation when explicitly enabled (e.g., corporate proxy with self-signed certs)
const shouldBypassSSL = process.env.BYPASS_SSL_VALIDATION === 'true';

const httpsAgent = shouldBypassSSL 
  ? new https.Agent({ rejectUnauthorized: false })
  : undefined; // Use default agent (with SSL validation)

if (shouldBypassSSL) {
  console.log('⚠️  SSL validation bypass enabled for Apps Script (corporate proxy mode)');
} else {
  console.log('🔒 SSL validation enabled for Apps Script (secure mode)');
}
  ...
  async scheduleEmail(emailData) {
    ...
    try {
      console.log('📤 Sending schedule request to Apps Script:', {
        to,
        cc: cc || 'none',
        bcc: bcc ? 'present' : 'none',
        scheduledTime: payload.scheduledTime,
        trackingId
      });

      // Use dynamic import for node-fetch (ESM module) with SSL bypass
      ...
      // Only add agent if SSL bypass is enabled
      if (httpsAgent) {
        fetchOptions.agent = httpsAgent;
      }

      const response = await fetchFn(this.appsScriptUrl, fetchOptions);
