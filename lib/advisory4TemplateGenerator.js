function generateAdvisory4EmailTemplate(advisory, customMessage = '', iocDetectionData = null) {
  try {
    ...
    if (iocDetectionData && iocDetectionData.client_name) {
      ...
      if (iocDetectionData.matches && iocDetectionData.matches.length > 0) {
        let matchesHtml = '';
        iocDetectionData.matches.forEach(match => {
          const timestamp = new Date(match.timestamp).toLocaleString();
          matchesHtml += `
                                        <tr style="border-bottom: 1px solid #FFE8D6;">
                                            <td style="padding: 10px; font-family: 'Roboto', Arial, sans-serif; font-size: 13px; color: #0B2233; font-family: monospace;">${match.ioc}</td>
                                            <td style="padding: 10px; font-family: 'Roboto', Arial, sans-serif; font-size: 13px; color: #0B2233; text-transform: uppercase;">${match.matched_field}</td>
                                            <td style="padding: 10px; font-family: 'Roboto', Arial, sans-serif; font-size: 13px; color: #0B2233;">${timestamp}</td>
                                        </tr>`;
        });
      ...
      // Remove the IOC detection section if not present
      template = template.replace(/\{%\s*if\s+ioc_detection\s+and\s+ioc_detection\.client_name\s*%\}[\s\S]*?\{%\s*endif\s*%\}/g, '');
      console.log('⚠️ [TEMPLATE GEN] IOC section removed (no iocDetectionData or client_name)');
    }
