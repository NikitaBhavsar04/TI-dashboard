function generateAdvisory4EmailTemplate(advisory, customMessage = '') {
  try {
    ...
    if (Array.isArray(advisory.exec_summary_parts)) {
      // Eagle Nest format - already split into parts
    ...
    let cvss_score = '';
    if (cvss) {
      if (typeof cvss === 'object' && Object.keys(cvss).length > 0) {
    ...
    // 4. CVSS Score section
    let cvss_html = cvss_score ? `<strong style="color: #0B2233;">CVSS:</strong> ${cvss_score}` : '<strong style="color: #999;">CVSS: N/A</strong>';
