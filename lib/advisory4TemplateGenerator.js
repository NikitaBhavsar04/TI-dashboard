  ...
  try {
    ...
    if (cvss) {
      if (Array.isArray(cvss) && cvss.length > 0) {
        ...
        cvss_vector = highest.vector || '';
        cvss_source = highest.source || '';
        ...
        cvss_vector = first.vector || '';
        cvss_source = first.source || '';
    ...
    console.log('📧 Generating email with ACTUAL ADVISORY DATA:', {
      ...
      cvss_score,
      cvss_vector: cvss_vector ? 'Yes' : 'No',
      cvss_source: cvss_source || 'N/A',
    ...
    if (cvss_score) {
      ...
      if (cvss_vector) {
        cvss_html += ` <span style="font-size: 12px; color: #666;">(${cvss_vector})</span>`;
      }
      if (cvss_source) {
        cvss_html += ` <span style="font-size: 11px; color: #888;">- ${cvss_source}</span>`;
      }
    } else {
