  ...
  try {
    ...
    if (mitre_array && mitre_array.length > 0) {
      mitre_array.forEach((row, index) => {
        ...
        const tactic = row.tactic || row.tacticName || row.name || row.Tactic || row.TACTIC || 'N/A';
        const techniqueId = row.id || row.techniqueId || row.technique_id || row['Technique ID'] || 'N/A';
        const technique = row.technique || (row.techniques && row.techniques[0]) || row.Technique || row.TECHNIQUE || 'N/A';
    ...
    // 7.5. IOC Detection Results section (for dedicated advisories only)
    console.log('🔍 [TEMPLATE GEN] iocDetectionData received:', JSON.stringify(iocDetectionData, null, 2));
    console.log('🔍 [TEMPLATE GEN] Has client_name:', !!iocDetectionData?.client_name);

    const hasMatches = iocDetectionData && iocDetectionData.client_name && Array.isArray(iocDetectionData.matches) && iocDetectionData.matches.length > 0 && Number(iocDetectionData.match_count || 0) > 0;

    if (hasMatches) {
      console.log('✅ [TEMPLATE GEN] Rendering IOC detection section for:', iocDetectionData.client_name);
      // Format the timestamp
      const checkedAt = new Date(iocDetectionData.checked_at).toLocaleString();

      // Replace simple variables
      template = template.replace(/\{\{\s*ioc_detection\.client_name\s*\}\}/g, iocDetectionData.client_name);
      template = template.replace(/\{\{\s*ioc_detection\.checked_at\s*\}\}/g, checkedAt);
      template = template.replace(/\{\{\s*ioc_detection\.match_count\s*\}\}/g, iocDetectionData.match_count);

      // Build the matches table
      if (iocDetectionData.matches && iocDetectionData.matches.length > 0) {
        let matchesHtml = '';
        iocDetectionData.matches.forEach((match, index) => {
          const timestamp = match.timestamp ? new Date(match.timestamp).toLocaleString() : '';
          const safeIoc = sanitizeIocVal(match.ioc || '');
          const safeField = sanitizeIocVal(match.matched_field || '');
          const hasContent = safeIoc || safeField || timestamp;
          if (!hasContent) return;
          const bgColor = index % 2 === 0 ? '#F7FAFD' : '#FFFFFF';
          
          matchesHtml += `
                                        <tr style="background-color: ${bgColor};">
                                            <td style="padding: 10px; border-bottom: 1px solid #D7DEE8; font-family: 'Roboto', Arial, sans-serif; font-size: 14px; color: #0B2233; font-family: monospace;">${safeIoc || '—'}</td>
                                            ...
                                            <td style="padding: 10px; border-bottom: 1px solid #D7DEE8; font-family: 'Roboto', Arial, sans-serif; font-size: 14px; color: #0B2233;">${timestamp || '—'}</td>
                                        </tr>`;
        });

        if (matchesHtml.trim()) {
          template = template.replace(/\{%\s*for\s+match\s+in\s+ioc_detection\.matches\s*%\}[\s\S]*?\{%\s*endfor\s*%\}/g, matchesHtml);
        } else {
          template = template.replace(/\{%\s*if\s+ioc_detection\s+and\s+ioc_detection\.client_name\s*%\}[\s\S]*?\{%\s*endif\s*%\}/g, '');
          console.log('⚠️ [TEMPLATE GEN] IOC section removed (no usable match rows)');
          return template;
        }
      }

      // Show the IOC detection section
      // Replace detection meta line; hide if no timestamps and no counts
      const detectionMeta = (checkedAt || iocDetectionData.match_count)
        ? `<p style="margin: 5px 0; font-family: 'Roboto', Arial, sans-serif; font-size: 14px; color: #5F6C7B; font-style: italic;">Detected at: ${checkedAt || '—'} | Total Matches: ${iocDetectionData.match_count ?? '—'}</p>`
        : '';
      template = template.replace(/<p style="margin: 5px 0;[^>]*>\s*Detected at:[\s\S]*?<\/p>/, detectionMeta);

      template = template.replace(/\{%\s*if\s+ioc_detection\s+and\s+ioc_detection\.client_name\s*%\}([\s\S]*?)\{%\s*endif\s*%\}/g, '$1');
      console.log('✅ [TEMPLATE GEN] IOC section included in template');
    } else {
      // Remove the IOC detection section if not present or no matches
      template = template.replace(/\{%\s*if\s+ioc_detection\s+and\s+ioc_detection\.client_name\s*%\}[\s\S]*?\{%\s*endif\s*%\}/g, '');
      console.log('⚠️ [TEMPLATE GEN] IOC section removed (no iocDetectionData, client_name, or matches)');
    }

    // Final verification
    const finalHasIoc = template.includes('IOC DETECTION ALERT');
    console.log(`🔍 [TEMPLATE GEN] Final template contains 'IOC DETECTION ALERT': ${finalHasIoc}`);
