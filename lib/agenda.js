...
agenda.define('send-scheduled-advisory-email', async (job, done) => {
  ...
  try {
    ...
    if (isValidObjectId) {
      // Intel Feed advisory - try MongoDB first
      advisory = await Advisory.findById(emailDoc.advisoryId).catch(() => null);
      if (!advisory) {
        console.log(`Advisory ${emailDoc.advisoryId} not found in MongoDB, checking Eagle Nest...`);
        // Fallback to Eagle Nest even for ObjectId-like IDs
        ...
        if (fs.existsSync(eagleNestPath)) {
          ...
          advisory = { ...eagleNestData, _id: emailDoc.advisoryId };
          console.log(`Found advisory in Eagle Nest: ${advisory.advisory_id || emailDoc.advisoryId}`);
        } else {
          console.error(`Advisory not found anywhere for email ${emailId}`);
    ...
    const trackingPixelUrl = `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/track-email/${emailDoc.trackingId}`;
