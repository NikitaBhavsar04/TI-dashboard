...
agenda.define('send-scheduled-advisory-email', async (job, done) => {
  ...
  try {
    ...
    // Safety check: if scheduled via Apps Script, don't send via Agenda
    if (emailDoc.schedulingMethod === 'apps-script' && emailDoc.appsScriptEmailId) {
      console.log(`⚠️ Email ${emailId} was scheduled via Apps Script (ID: ${emailDoc.appsScriptEmailId})`);
      console.log(`   Apps Script will handle sending. Skipping Agenda.js execution.`);
      return done();
    }
