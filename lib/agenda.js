...
// Agenda job: send the email immediately when schedule time arrives.
// Uses sendScheduledEmailById which has an atomic mutex — safe even if the
// built-in poller also fires at the same time (only one caller proceeds).
agenda.define('send-scheduled-advisory-email', async (job, done) => {
  const { emailId } = job.attrs.data;
  const emailIdStr = emailId ? emailId.toString() : null;
  console.log(`⏰ [AGENDA] Schedule time reached for email ${emailIdStr} — sending now`);
  if (!emailIdStr) {
    console.error('[AGENDA] emailId is missing from job data');
    return done(new Error('emailId missing'));
  }
  try {
    const { sendScheduledEmailById } = require('./sendScheduledEmailById');
    await sendScheduledEmailById(emailIdStr, ScheduledEmail);
    console.log(`✅ [AGENDA] Email ${emailIdStr} sent successfully`);
    done();
  } catch (err) {
    console.error(`❌ [AGENDA] Failed to send email ${emailIdStr}:`, err.message);
    done(err);
  }
});
