...
if not hf_cfg.get("enabled", False):
    raise RuntimeError("HuggingFace is disabled in config.yaml")

HF_MODEL = hf_cfg.get("model")
if not HF_MODEL:
    raise RuntimeError("huggingface.model missing in config.yaml")

if any(x in HF_MODEL.lower() for x in ["bart", "pegasus", "t5"]):
    raise RuntimeError(
        f"Model '{HF_MODEL}' is NOT a chat/instruct model."
    )
...
def call_llm(prompt: str, max_tokens: int = 2200) -> str:
    api_key = "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"
    if not api_key:
        raise RuntimeError("HF_API_KEY environment variable not set")

    client = OpenAI(base_url=HF_BASE, api_key=api_key)

    logger.info(f"[LLM] Using model: {HF_MODEL}")

    resp = client.chat.completions.create(
        model=HF_MODEL,
        messages=[
            {"role": "system", "content": "You are a senior SOC threat intelligence analyst."},
